...
- 【非常重要】workExperiences数组必须包含简历中提到的所有工作经历，即使是非家政行业的经历（如教师、工厂、销售、餐厅等）也必须完整保留，绝对不能删除或省略任何一段工作经历！
...
9. workExperiences（工作经历）数组:
   - 【重要】保留简历中提到的所有工作经历，包括非家政行业的经历（如教师、销售、工厂等），不要删除任何工作经历
...
10. experienceYears（工作年限）:
   ...
   - 否则根据家政/护理/育儿相关工作经历估算，非家政职业（如教师、售货员、健身教练）不算入家政年限，但工作经历仍需保留在workExperiences中
   - 返回数字
...
12. selfIntroduction: 综合以下内容生成50-150字的自我介绍：
   ...
   - "特长"/"擅长"部分的描述
   - 性格特点、工作态度等描述
   ⚠️ 必须使用第一人称视角，以阿姨本人的口吻自述（用"我"开头，不要出现姓名、"XX阿姨"、"该同志"等第三人称称呼）
   ⚠️ 若原文是第三人称描述（如"吕红丽性格温和有爱心"），需改写为第一人称（如"我性格温和有爱心"）
   ⚠️ 不要重复输出姓名、年龄、学历、婚姻状况、属相、星座、民族、籍贯等基础信息（这些字段已单独提取，避免冗余）
   ⚠️ 保留原文对性格、特长、技能、工作态度的实质内容，仅调整人称和口吻，不要过度改写

13. familySituation: 家庭情况描述（如"老公在家开货车，儿子上班"、"五口人，儿子都成家了"）
   ⚠️ 同样使用第一人称口吻（如"爱人在老家工作，孩子已经参加工作，家庭和睦无负担"）
   ⚠️ 不要带"家庭情况："前缀，只输出内容本身

17. recommendationReason: 根据阿姨的整体情况，站在内部推荐人的角度，写60-80字的推荐理由，必须以"【推荐理由】"开头，突出最核心的亮点，如"【推荐理由】李阿姨从事育儿嫂12年，专业扎实，持有高级育婴师和催乳师证书，有丰富的双胎护理经验，性格温和耐心，沟通能力强，深受客户好评，值得信赖推荐"
...
【返回JSON格式】
{
  ...
  "recommendationReason": "【推荐理由】60-80字推荐理由"
}`;
  }
...
只返回纯JSON（不要代码块）：{"category":"photo","reason":"简短理由"}`;
  ...
  async generateRecommendation(resume: {
    ...
    const prompt = `你是家政公司的推荐文案撰写助手，请根据以下简历信息，生成一段60-80字的推荐理由文案。
