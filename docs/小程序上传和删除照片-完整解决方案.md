# 小程序上传和删除照片 - 完整解决方案

## 📋 问题汇总

### 问题1：删除照片后仍然存在 ✅ 已解决

**原因**：小程序端使用了错误的删除方式（提交空数组）

**解决方案**：使用专门的删除 API

```
DELETE /api/resumes/miniprogram/:id/delete-file
```

### 问题2：上传一张照片显示两张 ✅ 已诊断

**原因**：小程序端的调用逻辑有问题（重复调用或重复添加到本地状态）

**解决方案**：参考 CRM 端的正确逻辑，只调用一次上传接口，只更新一次本地状态

## ✅ 后端逻辑确认

### 测试结果

我们对 CRM 后端进行了完整测试：

1. **删除 API 测试**：✅ 通过
   ```bash
   cd backend
   node test-delete-api.js
   ```
   结果：删除功能正常，字段同步正确

2. **上传 API 测试**：✅ 通过
   ```bash
   cd backend
   node test-upload-duplicate.js
   ```
   结果：上传一次只添加一次，不会产生重复

### 结论

**CRM 后端的逻辑是正确的，小程序端和 CRM 端使用相同的底层方法，不会产生重复。**

## 🎯 API 接口说明

### 1. 上传文件

```
POST /api/resumes/miniprogram/:id/upload-file
```

**参数**：
- `id`（URL路径）：简历ID
- `file`（FormData）：文件
- `type`（FormData）：文件类型

**文件类型**：
- `certificate` - 技能证书
- `personalPhoto` - 个人照片
- `medicalReport` - 体检报告
- `idCardFront` - 身份证正面
- `idCardBack` - 身份证背面
- `selfIntroductionVideo` - 自我介绍视频
- `confinementMealPhoto` - 月子餐照片
- `cookingPhoto` - 烹饪照片
- `complementaryFoodPhoto` - 辅食添加照片
- `positiveReviewPhoto` - 好评展示照片

**响应**：
```json
{
  "success": true,
  "data": {
    "fileUrl": "https://...",
    "fileType": "certificate",
    "fileName": "photo.jpg",
    "fileSize": 123456,
    "resumeId": "xxx"
  },
  "message": "文件上传成功"
}
```

### 2. 删除文件

```
DELETE /api/resumes/miniprogram/:id/delete-file
```

**参数**：
```json
{
  "fileUrl": "https://...",
  "fileType": "certificate"
}
```

**响应**：
```json
{
  "success": true,
  "data": {
    "resumeId": "xxx",
    "deletedFileUrl": "https://...",
    "fileType": "certificate"
  },
  "message": "文件删除成功"
}
```

## 💻 小程序端完整实现

```javascript
// pages/resume/edit.js
Page({
  data: {
    resumeId: '',
    certificateUrls: [],
    uploading: false
  },

  onLoad(options) {
    this.setData({ resumeId: options.id });
    this.loadResume();
  },

  // 加载简历
  async loadResume() {
    try {
      const res = await wx.request({
        url: `${API_BASE_URL}/api/resumes/miniprogram/${this.data.resumeId}`,
        method: 'GET',
        header: {
          'Authorization': `Bearer ${wx.getStorageSync('token')}`
        }
      });

      if (res.data.success) {
        this.setData({
          certificateUrls: res.data.data.certificateUrls || []
        });
      }
    } catch (error) {
      console.error('加载简历失败:', error);
    }
  },

  // 上传证书
  async onUploadCertificate() {
    if (this.data.uploading) {
      wx.showToast({ title: '正在上传中...', icon: 'none' });
      return;
    }

    try {
      const res = await wx.chooseImage({
        count: 1,
        sizeType: ['compressed'],
        sourceType: ['album', 'camera']
      });

      await this.uploadFile(res.tempFilePaths[0], 'certificate');
    } catch (error) {
      console.error('选择图片失败:', error);
    }
  },

  // 上传文件
  async uploadFile(filePath, fileType) {
    this.setData({ uploading: true });
    wx.showLoading({ title: '上传中...', mask: true });

    try {
      const uploadRes = await wx.uploadFile({
        url: `${API_BASE_URL}/api/resumes/miniprogram/${this.data.resumeId}/upload-file`,
        filePath: filePath,
        name: 'file',
        formData: { type: fileType },
        header: {
          'Authorization': `Bearer ${wx.getStorageSync('token')}`
        }
      });

      const data = JSON.parse(uploadRes.data);
      wx.hideLoading();

      if (data.success) {
        wx.showToast({ title: '上传成功', icon: 'success' });
        
        // ✅ 只添加一次到本地状态
        this.setData({
          certificateUrls: [...this.data.certificateUrls, data.data.fileUrl]
        });
      } else {
        wx.showToast({ title: data.message, icon: 'none' });
      }
    } catch (error) {
      wx.hideLoading();
      console.error('上传失败:', error);
      wx.showToast({ title: '上传失败', icon: 'none' });
    } finally {
      this.setData({ uploading: false });
    }
  },

  // 删除证书
  async onDeleteCertificate(e) {
    const { url } = e.currentTarget.dataset;

    try {
      const confirmRes = await wx.showModal({
        title: '确认删除',
        content: '确定要删除这张证书照片吗？'
      });

      if (!confirmRes.confirm) return;

      wx.showLoading({ title: '删除中...', mask: true });

      const res = await wx.request({
        url: `${API_BASE_URL}/api/resumes/miniprogram/${this.data.resumeId}/delete-file`,
        method: 'DELETE',
        header: {
          'Authorization': `Bearer ${wx.getStorageSync('token')}`,
          'Content-Type': 'application/json'
        },
        data: {
          fileUrl: url,
          fileType: 'certificate'
        }
      });

      wx.hideLoading();

      if (res.data.success) {
        wx.showToast({ title: '删除成功', icon: 'success' });
        
        // 从本地状态中移除
        this.setData({
          certificateUrls: this.data.certificateUrls.filter(u => u !== url)
        });
      } else {
        wx.showToast({ title: res.data.message, icon: 'none' });
      }
    } catch (error) {
      wx.hideLoading();
      console.error('删除失败:', error);
      wx.showToast({ title: '删除失败', icon: 'none' });
    }
  },

  // 预览图片
  onPreviewImage(e) {
    const { url } = e.currentTarget.dataset;
    wx.previewImage({
      current: url,
      urls: this.data.certificateUrls
    });
  }
});
```

## 🎨 WXML 模板

```xml
<view class="container">
  <view class="section">
    <view class="section-title">技能证书</view>
    
    <view class="image-list">
      <!-- 已上传的证书 -->
      <block wx:for="{{certificateUrls}}" wx:key="index">
        <view class="image-item">
          <image 
            src="{{item}}" 
            mode="aspectFill"
            bindtap="onPreviewImage"
            data-url="{{item}}"
          />
          <view 
            class="delete-btn" 
            bindtap="onDeleteCertificate"
            data-url="{{item}}"
          >
            <text>×</text>
          </view>
        </view>
      </block>
      
      <!-- 上传按钮 -->
      <view 
        class="upload-btn" 
        bindtap="onUploadCertificate"
        wx:if="{{!uploading}}"
      >
        <text class="icon">+</text>
        <text class="text">添加证书</text>
      </view>
    </view>
  </view>
</view>
```

## ⚠️ 重要注意事项

### 1. 防止重复上传

```javascript
// ✅ 使用 uploading 标志位
if (this.data.uploading) {
  wx.showToast({ title: '正在上传中...', icon: 'none' });
  return;
}
```

### 2. 只调用一次上传接口

```javascript
// ✅ 正确
async onUploadCertificate() {
  const res = await wx.chooseImage({...});
  await this.uploadFile(res.tempFilePaths[0], 'certificate');  // 只调用一次
}

// ❌ 错误
async onUploadCertificate() {
  const res = await wx.chooseImage({...});
  await this.uploadFile(res.tempFilePaths[0], 'certificate');  // 第一次
  await this.uploadFile(res.tempFilePaths[0], 'certificate');  // 第二次（错误！）
}
```

### 3. 只更新一次本地状态

```javascript
// ✅ 正确
if (data.success) {
  this.setData({
    certificateUrls: [...this.data.certificateUrls, data.data.fileUrl]
  });
}

// ❌ 错误
if (data.success) {
  this.setData({
    certificateUrls: [...this.data.certificateUrls, data.data.fileUrl]
  });
  this.setData({
    certificateUrls: [...this.data.certificateUrls, data.data.fileUrl]  // 重复！
  });
}
```

### 4. 不要在上传后调用更新接口

```javascript
// ✅ 正确：上传接口已经保存到数据库了
async uploadFile(filePath, fileType) {
  const uploadRes = await wx.uploadFile({...});
  const data = JSON.parse(uploadRes.data);
  
  if (data.success) {
    // 只更新本地状态
    this.setData({
      certificateUrls: [...this.data.certificateUrls, data.data.fileUrl]
    });
  }
}

// ❌ 错误：不要再调用更新接口
async uploadFile(filePath, fileType) {
  const uploadRes = await wx.uploadFile({...});
  
  // 又调用更新接口（错误！会导致重复）
  await wx.request({
    url: `/api/resumes/miniprogram/${this.data.resumeId}`,
    method: 'PATCH',
    data: { certificateUrls: [...] }  // ❌ 不要这样做
  });
}
```

## 📚 相关文档

1. **小程序端使用指南**
   - `docs/给小程序AI的上传API使用指南-简洁版.md` - 快速开始
   - `docs/小程序端文件上传API使用方法.md` - 完整文档
   - `docs/给小程序AI的删除照片指南.md` - 删除功能

2. **问题诊断**
   - `docs/小程序上传照片重复问题-诊断报告.md` - 问题分析
   - `docs/小程序删除照片问题-最终解决方案.md` - 删除问题

3. **测试脚本**
   - `backend/test-upload-duplicate.js` - 上传重复测试
   - `backend/test-delete-api.js` - 删除功能测试

## 🎯 总结

| 功能 | API 接口 | 状态 |
|------|---------|------|
| 上传文件 | `POST /api/resumes/miniprogram/:id/upload-file` | ✅ 正常 |
| 删除文件 | `DELETE /api/resumes/miniprogram/:id/delete-file` | ✅ 正常 |
| 后端逻辑 | 与 CRM 端一致 | ✅ 已验证 |
| 测试结果 | 不会产生重复 | ✅ 已通过 |

**问题根源**：小程序端的调用逻辑有问题，不是 CRM 后端的问题。

**解决方案**：参考上面的完整代码示例，修复小程序端的上传和删除逻辑。

---

**最后更新**：2025-01-11  
**测试状态**：✅ 已通过  
**可用性**：✅ 生产环境可用

