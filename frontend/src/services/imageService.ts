...
export class ImageService {
  ...
  static extractProvince(address: string): string | null {
    ...
    // 尝试简写匹配（按简写长度降序排列，避免"山西"匹配到"山"）
    const sortedShortNames = Object.keys(shortNameMap).sort((a, b) => b.length - a.length);
    for (const shortName of sortedShortNames) {
      if (address.startsWith(shortName)) {
