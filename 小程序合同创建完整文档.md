...
**单个合同**：`POST /api/contracts/miniprogram/sync-esign-status/:id`

**批量同步**：`POST /api/contracts/miniprogram/sync-all-esign-status`
...
**换人合同特点**：
- ✅ 自动复制原合同的客户信息、服务地址等
- ✅ 自动继承原合同的 `templateParams`（服务时间、休息方式等）
...
**自动同步**：合同签署完成后（状态变为 `active`），后端自动触发保险同步

**换人合同**：签署完成后自动触发保险换人，将原阿姨的保险换成新阿姨
